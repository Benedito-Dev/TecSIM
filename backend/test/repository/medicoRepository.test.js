...
describe('MedicoRepository', () => {
  ...
  describe('findById', () => {
    it('deve retornar médico ativo pelo id', async () => {
      db.query.mockResolvedValue({ rows: [mockMedicoData] });
  ...
  describe('findByCrm', () => {
    it('deve retornar médico ativo pelo CRM', async () => {
      db.query.mockResolvedValue({ rows: [mockMedicoData] });
  ...
  describe('update', () => {
    it('deve atualizar os dados de um médico', async () => {
      ...
      db.query
        ...
        .mockResolvedValueOnce({ rows: [updatedMock] });   // UPDATE
    ...
    it('deve lançar erro se CRM já estiver em uso por outro médico', async () => {
      ...
      db.query
        .mockResolvedValueOnce({ rows: [mockMedicoData] }) // findById
        .mockResolvedValueOnce({ rows: [outroMedico] });   // findByCrm
  ...
  describe('activate', () => {
    it('deve ativar um médico', async () => {
      // GARANTIR que o médico mockado está com ativo: true
      const medicoAtivado = { 
        ...mockMedicoData, 
        ativo: true  // ← EXPLÍCITO e VERIFICADO
      };
      
      console.log('Mock médico ativado:', medicoAtivado); // ← DEBUG
