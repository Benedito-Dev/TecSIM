...
class PacienteService {
  ...
  static async update(id, dados) {
    ...
    const { senhaAtual, ...dadosParaAtualizar } = dados;

    // 1. Verificação da senha atual
    if (!senhaAtual) {
      throw new ValidationError('A senha atual é obrigatória para atualizar os dados.');
    }

    const pacienteParaVerificar = await repository.findById(id, true); // true para retornar a senha
    if (!pacienteParaVerificar) throw new NotFoundError('Paciente não encontrado.');

    const senhaCorreta = await bcrypt.compare(senhaAtual, pacienteParaVerificar.senha);
    if (!senhaCorreta) {
      throw new ValidationError('Senha atual incorreta.');
    }

    // 2. Validação de email/cpf (lógica existente)
    if (dadosParaAtualizar.email && dadosParaAtualizar.email !== pacienteParaVerificar.email) {
      const existingByEmail = await repository.findByEmail(dadosParaAtualizar.email);
      if (existingByEmail) {
        throw new ConflictError('Email já cadastrado.');
      }
    }

    if (dadosParaAtualizar.cpf && dadosParaAtualizar.cpf !== pacienteParaVerificar.cpf) {
      const existingByCPF = await repository.findByCPF(dadosParaAtualizar.cpf);
      if (existingByCPF) {
        throw new ConflictError('CPF já cadastrado.');
      }
    }

    // 3. Remover campos sensíveis que não devem ser atualizados diretamente
    delete dadosParaAtualizar.senha;
    delete dadosParaAtualizar.id_paciente;
    delete dadosParaAtualizar.data_cadastro;

    // 4. Atualizar os dados
    const pacienteAtualizado = await repository.update(id, dadosParaAtualizar);
    return pacienteAtualizado;
  }
